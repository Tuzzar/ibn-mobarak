...
create policy "Staff read app settings" on public.app_settings for select to authenticated using (public.is_staff(auth.uid()));
...
create policy "Staff read courier checks" on public.courier_checks for select to authenticated using (public.is_staff(auth.uid()));

drop trigger if exists app_settings_updated_at on public.app_settings;
create trigger app_settings_updated_at before update on public.app_settings for each row execute function public.update_updated_at();
drop trigger if exists courier_checks_updated_at on public.courier_checks;
create trigger courier_checks_updated_at before update on public.courier_checks for each row execute function public.update_updated_at();